...
function passwordCheck4() {
    ...
    if (password === "Miem605750887ToOpinionIndex") {//左の""には照合させたいパスワードを入れる
        window.location.href = "../administrator/Opinion/Index";
                                //""内は本来のリンク先URL
    } else {
...
function passwordCheck5() {
    ...
    // 仮の認証ロジックを示します。実際のアプリケーションでは適切な認証手段を使用してください。
    if (password === "Miem605750887ToOpinionCheckedIndex") {//左の""には照合させたいパスワードを入れる
        window.location.href = "../administrator/Opinion/CheckedIndex";//左""内は本来のリンク先URL
    } else {
        // パスワードが異なる場合、エラーメッセージを表示
